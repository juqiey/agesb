...
@section('content')
                ...
                <div class="col-12">
                    <div class="card shadow-sm">
                        ...
                        <div class="card-header pb-0 bg-light">
                            <h4 class="mb-0 text-center">Approve SSA</h4>
                        </div>

                        <div class="card-body px-4 pt-4 pb-4">

                            <form action="{{ route('ssa.approve.update', $ssa) }}" method="POST" enctype="multipart/form-data" onsubmit="console.log('Submitting form'); return true;">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="mb-4 p-3 border rounded bg-white">
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input disabled name="ssa_no" class=" form-control" value="{{ old('ssa_no',$ssa->ssa_no) }}" required>
                                        </div>

                                        <div class="col-md-6 mb-3">
                                            <label class="form-label">Location</label>
                                            <input disabled name="location" class=" form-control" value="{{ old('location',$ssa->location) }}" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select disabled name="vessel" class="form-select" required>
                                                @foreach($vessels as $code=>$name)
                                                    <option value="{{ $code }}" {{ $ssa->vessel==$code?'selected':'' }}>{{ $name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input disabled type="date" name="ssa_date" class="form-control" value="{{ \Carbon\Carbon::parse($ssa->date)->format('Y-m-d') }}" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row mb-0">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            @foreach($departments as $k=>$v)
                                                <div class="form-check form-check-inline">
                                                    <input disabled class="form-check-input" type="radio" name="department" value="{{ $k }}" {{ $ssa->department==$k?'checked':'' }}>
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            @foreach($ssaRaisedOptions as $option)
                                                <div class="form-check form-check-inline">
                                                    <input disabled class=" form-check-input" type="radio" name="ssa_raised" value="{{ $option }}" {{ $ssa->ssa_raised==$option?'checked':'' }}>
                                ...
                                <div class="mb-4 p-3 border rounded bg-white">
                                    ...
                                    @if($ssa->doc_url)
                                        <div class="mt-2 align-items-center">
                                            <a href="{{ asset($ssa->doc_url) }}" target="_blank" class="btn btn-info">Current Attachment</a>
                                        </div>
                                    @else
                                        <div class="mt-2">
                                ...
                                <div class="mb-4 p-3 border rounded bg-white">
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-12">
                                            ...
                                            <input type="text" name="approved_remark" class="form-control"
                                                   value="{{ old('approved_remark', $ssa->approved_remark ?? '') }}"
                                    ...
                                    <div class="d-flex align-items-center gap-3">
                                        ...
                                        <div class="form-check form-switch">
                                            <input type="checkbox" class="form-check-input" id="approveSwitch"
                                                {{ old('approved_status', $ssa->approved_status) === 'APPROVED' ? 'checked' : '' }}>

                                            <span id="approveBadge" class="badge {{ $ssa->approved_status === 'APPROVED' ? 'bg-success' : 'bg-warning' }}">
                                                {{ old('approved_status', $ssa->approved_status) ?? 'PENDING' }}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <input type="hidden" name="approved_status" id="approvedStatus"
                                           value="{{ old('approved_status', $ssa->approved_status) ?? 'PENDING' }}">
    ...
    @push('scripts')
        <script>
            $('#approveSwitch').on('change', function () {
